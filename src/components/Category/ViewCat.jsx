...
const [loading, setLoading] = useState(true);
  ...
  const initPage = Math.ceil(handleDataLength(categorylist) / perPage);

  const handleNext = () => {
    let currentPage = page;
    let lastValue = lastIndex;

    if (handleDataLength(categorylist) > lastIndex) {
      currentPage += 1;
      lastValue += perPage;
  ...
  const handlePrev = () => {
    ...
    if (handleDataLength(categorylist) > initIndex) {
      if (initIndex != 0) {
        currentPage -= 1;
        initValue -= perPage;
        lastValue -= perPage;
  ...
  console.log("Status: " + handleDataLength(categorylist));
